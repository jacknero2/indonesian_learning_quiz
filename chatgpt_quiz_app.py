...
class QuizApp(tk.Tk):
    def __init__(self):
        ...
        self.happy_img = tk.PhotoImage(file="happy_penguin.png").subsample(4, 4)
        self.sad_img = tk.PhotoImage(file="sad_penguin.png").subsample(4, 4)
    ...
    def show_splash_screen(self):
        ...
        # Set the splash screen dimensions and center it
        ...
        # Create a canvas to display the image and overlay text
        canvas = tk.Canvas(self.splash, width=width, height=height, highlightthickness=0)
        canvas.pack()

        # Load and display the splash image
        try:
            self.splash_img = tk.PhotoImage(file="open.png")
        except Exception as e:
        ...
        # Overlay the bold "PengLang" text at the top center
    ...
    def build_config_screen(self):
        """Screen to let the user set quiz options, then start."""
        ...
        tk.Checkbutton(self.config_frame, text="Answer in Indonesian", variable=self.target_var,
                       bg="light blue", fg="black").pack(anchor="w")
    ...
    def start_quiz(self):
        """Grab user settings, do optional search, then begin the quiz."""
        ...
        if self.keyword:
            found = search(self.keyword, data)
            if found[0]:
    ...
    def build_quiz_screen(self):
        ...
        if self.mnemonic:
            ...
            self.mnemonic_entry = tk.Entry(self.quiz_frame, textvariable=self.mnemonic_var, width=40,
                                           bg="white", fg="black")
        ...
        # New image label to show happy/sad penguin
    ...
    def show_question(self):
        # Clear previous image and reset answer state
        ...
        if self.target:
            question_text = f"What is '{data[idx][2]}' in Indonesian?"
        else:
            question_text = f"What is '{data[idx][1]}' in English?"
    ...
    def submit_answer(self):
        # Two-step submission: first click checks the answer,
        ...
        if not self.answer_checked:
            ...
            if self.target:
                correct_answer = data[idx][1].lower()
            else:
                correct_answer = data[idx][2].lower()

            if user_answer == correct_answer:
                self.feedback_label.config(text="Correct!", fg="green")
                self.score += 1
                play_local_sound("correct.mp3")
                ...
                if data[idx] not in self.review_words:
                    self.review_words.append(data[idx])
                play_local_sound("incorrect.mp3")
                self.image_label.config(image=self.sad_img)

            open_audio(self.audio, self.audio_ai, data[idx][1])

            if self.mnemonic:
                new_mn = self.mnemonic_var.get().strip()
                if new_mn:
                    overwrite_mnemonic(data[idx][1], new_mn)

            # Mark that the answer has been checked
            self.answer_checked = True

        else:
            # On second click, move to the next question.
